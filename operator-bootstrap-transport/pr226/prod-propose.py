#!/usr/bin/env python3
"""PROD fixed-source proposal: one template upload and one CFN change set.

No execution, IAM changes, stack-policy updates or mutation retries. Run only
after exact PROD6 imports, native IN_SYNC drift and permanent guard.
...
Usage: python3 -I propose.py prod HELPER SOURCE PACKETS RECEIPTS AWS
...
PARTITION_HASHES = {
    "prod": "840607ed47847384131c43f058b1d5db0d9b47b1661645748f3fec168dd8f0b2"
}
RELEASED_STATE = {
    "prod": ("tJ6HbWVZBsGpK.bPrNqW4p3YYfu8zh1Y", '"d59097a7d19a46e1d7dc8b8ca2aa28f9"')
}
SEED_KEYS = {"prod": "2d42d715-1452-4766-8cfa-020b53288443"}
...
def partition_for(directory, environment, account, full):
    ...
    assert environment == "prod" and account == "933245420672"
    counts = (6, 52)
    ...
    assert sum(x["resource_type"] == "AWS::IAM::ManagedPolicy" for x in additions) == 49
    assert sum(x["resource_type"] == "AWS::IAM::Role" for x in additions) == 3
    for row in additions:
        if row["resource_type"] == "AWS::IAM::Role":
            assert full["Resources"][row["logical_resource_id"]]["Properties"][
...
def validate_quota(quota, full, attachment_sets):
    assert quota["Roles"] + 3 <= quota["RolesQuota"]
    assert quota["Policies"] + 49 <= quota["PoliciesQuota"]
    assert quota["PolicyVersionsInUse"] + 49 <= quota["PolicyVersionsInUseQuota"]
    assert all(
        len(values) <= min(10, quota["AttachedPoliciesPerRoleQuota"])
        for values in attachment_sets.values()
    )
    for definition in full["Resources"].values():
        props = definition["Properties"]
        if definition["Type"] == "AWS::IAM::ManagedPolicy":
            assert len(
                json.dumps(props["PolicyDocument"], separators=(",", ":"))
            ) <= min(6144, quota["PolicySizeQuota"])
        else:
            assert len(props["ManagedPolicyArns"]) <= min(
                10, quota["AttachedPoliciesPerRoleQuota"]
            )
            assert (
                len(
                    json.dumps(props["AssumeRolePolicyDocument"], separators=(",", ":"))
                )
                <= quota["AssumeRolePolicySizeQuota"]
            )
...
def main():
    ...
    stacks = {"prod": "2d3d1960-ab23-11f1-b7dc-02b3dc85e917"}
    ...
    token = "pr226-prod-enroll-" + uuid.uuid4().hex
    ...
    def call(service, operation, *arguments):
        assert (
            ...
            or (
                service == "s3api"
                and operation
                in {"head-object", "list-objects-v2", "put-object", "get-object"}
            )
        )
        endpoint = (
    ...
    def preconditions(stage):
        ...
        summary = call("iam", "get-account-summary")["SummaryMap"]
        needed = (
            "Roles",
            "RolesQuota",
            "Policies",
        ...
        quota = {name: summary[name] for name in needed}
        assert all(type(value) is int and value >= 0 for value in quota.values())
        save(stage + "-iam-quota.json", quota)
        attachment_sets = {}
        for row in partition["required_additions"]:
            props = full["Resources"][row["logical_resource_id"]]["Properties"]
            if row["resource_type"] == "AWS::IAM::ManagedPolicy":
                for name in props.get("Roles", []):
                    attachment_sets.setdefault(name, set()).add(
                        row["expected_physical_arn"]
                    )
        for index, (name, adds) in enumerate(sorted(attachment_sets.items())):
            current = paginated(
                call,
                "iam",
                "list-attached-role-policies",
                ...
                "AttachedPolicies",
                save,
                f"{stage}-role-attachments-{index}",
            )
            arns = [x["PolicyArn"] for x in current]
            assert len(arns) == len(set(arns))
            adds.update(arns)
        validate_quota(quota, full, attachment_sets)
