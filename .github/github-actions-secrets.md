...
Account IDs are independently pinned repository variables, checked before role assumption.
Configuration suffixes do not replace protected job environments. Keep `test`,
`test-preview`, `prod-preview`, `prod`, `governance`, `governance-preview`,
`operations-alert-reconcile`, and the evidence environment under the installed
main-only controls. Do not delete environment protection during variable cleanup.
