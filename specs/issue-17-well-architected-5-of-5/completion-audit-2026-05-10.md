...
| Head SHA | `42614be3f1f9a2d17bfdbde2936089e1b5aa9594` |
...
| Check PR #22 code and state. | `gh pr view 22` on the current head reports open, approved, not draft, and `mergeStateStatus=CLEAN`; hosted checks passed for `42614be`, with only expected unprivileged Preview/IAM jobs skipped. | PR control-plane evidence is current and green for repository-owned checks. | Done |
...
The collector final scores on the current head are:
