...
| Check whole project code. | Local validation commands and hosted checks: targeted ruff, format check, focused Well-Architected question/pillar validation tests, `make verify-well-architected-questions`, `make test-unit`, hosted quality/security/policy/mutation/local-battery checks, and `git diff --check`. | Repository-owned code validation is green on the latest audited head; latest local validation covered score-ID, score-summary, evidence status, external-control ID, unresolved-control summary, per-question pillar drift rejection, and branch-protection required-check contract drift across the helper, collector, guardrail docs, quality-gate docs, external-control evidence, and operating evidence. Latest local unit run reported `294 passed` and 100% reported coverage. | Done for latest audited run |
