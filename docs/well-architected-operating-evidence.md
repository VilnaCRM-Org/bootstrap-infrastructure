...
| --- | --- | --- | --- |
| Failed preview or destructive diff | Inspect the matching GitHub job, run `make test-guardrails-unprivileged`, then run privileged preview only with approved OIDC metadata. | Fix code or remove the critical destructive action; labels cannot override this gate. | Workflow URL, head SHA, preview summary, reviewer. |
