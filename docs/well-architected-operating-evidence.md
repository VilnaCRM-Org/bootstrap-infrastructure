...
| Production apply request | Verify the reviewed commit SHA, successful test deploy on `main`, production preview, destructive diff, IAM validation, and protected environment approval. | Do not apply until `prod` environment approval and reviewed SHA match. | Environment approval metadata, workflow URL, commit SHA, approver. |
