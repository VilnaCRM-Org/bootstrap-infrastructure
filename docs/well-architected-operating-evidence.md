...
infrastructure Well-Architected review refreshed on 2026-06-07. It records current
owners, review cadence, fallback actions, and non-secret evidence locations for
controls that can be owned in this repository. The current score-evidence
refresh is dated 2026-06-07.
...
owner attestations before retaining any final 5/5 claim.
...
| Backup, restore, drift, and DR | SRE | SRE | Monthly backup review; quarterly restore or DR drill | Use `docs/incident-drill-evidence-2026-05-09.md`; treat restore or DR readiness as stale after 90 days without a successful drill. |
| Cost controls and transfer review | `platform-maintainers` acting as FinOps owner for the test workload | Maintainer | Monthly and per catalog expansion | Use `docs/finops-review-2026-05-09.md`; block future cost claims when thresholds, reports, or active tag evidence are stale. |
...
| Security account attestations | #28 | `Kravalg`, `pixelTM`, `vilnacrm` | `security-account-attestation-2026-06-07-approved.json` records current human-access approval, `activeKeyDecision=approved_exception` for one active IAM user access key, and `permissionsBoundaryDecision=not_required` for account `891377212104`; refresh before 2026-07-07 or IAM scope changes. |
...
| AWS identity and metadata collector | `make report-well-architected-evidence` collector output passed identity, alert topic, CloudTrail, restore, IAM aggregate access, and fanout checks for the current PR evidence refresh on 2026-06-07. | Current metadata signal. | Re-run after every new push or account change. |
...
| Security account services | Repository code provisions GuardDuty, Security Hub, AWS Config recorder, and the Config delivery bucket; `docs/security-operating-evidence.md` records identity, permission, wildcard, boundary, live metadata, no-drift evidence, and the current June 7 security-owner attestation. | Current for test-account Security evidence. | Refresh after security-service, recorder, delivery, IAM, or region changes. |
...
| Incident and DR drill freshness | Latest incident/DR scenario drill no older than 90 days. | `docs/incident-drill-evidence-2026-05-09.md` records state, replica, KMS, IAM/OIDC, logging, backup, and workflow scenarios with live metadata checks. | SRE | Quarterly | Keep OPS10, SEC10, REL11, REL12, and affected DR claims below 5/5 after 90 days without a fresh drill. |
| Drift freshness | Shared-stack drift evidence no older than 24 hours. | Local `make test-drift` previously passed with `86 unchanged`; hosted drift remains an apply-path evidence requirement outside the PR check suite. | SRE | Daily for shared stacks | Treat merge readiness as blocked when hosted drift evidence is absent for apply paths. |
| Guardrail health | Same-repo required checks pass and are not skipped. | The latest collector and hosted-check refresh confirm the current PR head completed real test preview, destructive gate, cost proxy, IAM validation, Local Battery, mutation, security scans, and quality gates. | Maintainer | Per PR | Do not merge until hosted checks pass or admin records a temporary exception. |
...
| Alert route freshness | Operations SNS route has a subscription, inventory, route test, and monthly observation record. | `docs/alert-routing-evidence.md` records four enabled EventBridge rules, SNS targets, no runtime alarms/dashboards, a successful SNS-to-SQS probe, and the `make report-alert-route-observation` path for monthly review records; downstream human route remains external. | SRE | Monthly | Keep organizational consumption and escalation claims below 5/5 until downstream route is confirmed and current observation history exists. |
| Cost alert readiness | Budget and anomaly thresholds reviewed in the last 30 days. | `docs/finops-review-2026-05-09.md` records approved thresholds, active allocation tags, SNS route, month-to-date cost, and transfer review. | `platform-maintainers` | Monthly | Keep cost questions below 5/5 when the FinOps review is older than 30 days or cost/transfer thresholds are exceeded without owner approval. |
...
| Replica lag or replica unavailable | Check S3 replication configuration, sampled object replication status, and replication metrics when available. | Keep primary state read-only until lag is understood; restore from primary or backup if replica is unusable. | Bucket names, replication rule IDs, sampled object metadata, last successful backup, owner decision. |
...
| Data transfer | Cross-region replication exists. | Current transfer evidence and thresholds are recorded in `docs/finops-review-2026-05-09.md`; refresh before increasing replicated data classes or regions. |
