...
| Runner image vulnerability posture | `docs/compute-runner-evidence.md` records immutable ECR repository metadata, scan-on-push, and current scan findings: 4 critical and 18 high. | SEC6 remains blocked until the runner image is rebuilt/removed or exceptions are approved. | Platform owner refreshes the runner image scan before compute claims can pass. |
