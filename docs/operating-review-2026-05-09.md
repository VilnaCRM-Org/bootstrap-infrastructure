...
| Security external attestations | Repository admin plus security reviewer | GuardDuty, Security Hub, AWS Config, and aggregate IAM posture are covered by current metadata and `security-account-attestation-2026-06-07-approved.json`; the June 7 record contains one active IAM user access key under its approved exception and the historical `permissionsBoundaryDecision=not_required` decision (superseded for the new controller roles by the mandatory boundaries in `specs/security-completion/architecture.md`). | Refresh the security-owner attestation before 2026-07-07 or after IAM/user/permission scope changes. |
