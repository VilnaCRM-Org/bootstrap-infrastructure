...
| Default-branch Dependabot alerts | Security reviewer plus maintainer | Five high-severity `GitPython` alerts remain open for `uv.lock`; the collector now fails `github_dependabot_alerts` while those alerts are open. | Merge the patched lockfile to `main` or record owner-approved exceptions for any remaining alerts. |
