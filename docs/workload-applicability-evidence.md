...
| Security account controls | GuardDuty, Security Hub, AWS Config recorder, and Config delivery. | Account/region, delivery bucket, recorder role. | Security posture and configuration inventory metadata. | Missing post-apply posture keeps security account controls unresolved. | Security reviewer plus SRE | `pulumi/infra/security_account_controls.py` |
