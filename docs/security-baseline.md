...
Use `make pulumi-plan` before `make pulumi-up-plan`, and keep reviewable PRs as
the normal path for infrastructure changes. An apply that is not tied to the
reviewed saved plan is much harder to trust later.
