...
| AWS Config snapshots | Dedicated S3 bucket with default SSE-S3, SSE-C blocked, TLS-only bucket policy, versioning, lifecycle expiration, and public-access block. | Config snapshots are configuration metadata; SSE-S3 plus scoped delivery and retention controls are sufficient for this workload. | `pulumi/infra/security_account_controls.py`, post-apply Config metadata evidence. | Keep security account controls unresolved until recorder, delivery channel, and bucket posture are proven after apply. |
