...
import pytest
from infra import automation, governance, governance_automation, platform_iam
...
def test_governor_alias_operations_keep_target_key_conditions():
    document = json.loads(
        governance_automation.governance_repo_storage_policy(inputs(), REPO, apply=True)
    )
...
def test_governor_rejects_config_role_names_that_cannot_be_created():
    with pytest.raises(ValueError, match="config read role name"):
        governance_automation._role_resources(inputs(), replace(REPO, name="a" * 42))
