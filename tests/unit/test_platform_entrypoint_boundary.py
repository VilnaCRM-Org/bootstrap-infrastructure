...
@pytest.mark.parametrize(
    "identifier", [None, "", "12345", "1234567890123", "１２３４５６７８９０１２"]
)
def test_account_guard_rejects_malformed_equal_values(identifier):
    """Matching malformed values never substitute for a valid account boundary."""
    from infra.iam.account import assert_bootstrap_account

    with pytest.raises(ValueError, match="12-digit awsAccountId"):
        assert_bootstrap_account(identifier, identifier)
