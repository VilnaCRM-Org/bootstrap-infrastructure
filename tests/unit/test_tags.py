...
def test_base_tags_with_app(monkeypatch):
    ...
    monkeypatch.setattr(settings, "data_classification", "internal")
    monkeypatch.setattr(settings, "criticality", "high")
    monkeypatch.setattr(settings, "retention_class", "standard")
    ...
    assert tags["DataClassification"] == "internal"  # nosec B101
    assert tags["Criticality"] == "high"  # nosec B101
    assert tags["RetentionClass"] == "standard"  # nosec B101
