...
def test_mutation_target_pulumi_secrets_component(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda repo_name, region: f"awskms://alias/pulumi-{repo_name}-test-secrets?region={region}",
    )

    secrets = pulumi_secrets.PulumiSecretsKeys("pulumi-secrets", repositories=repos)

    assert (
        component_init["resource_type"] == "bootstrap:kms:PulumiSecretsKeys"
    )  # nosec B101
    assert component_init["name"] == "pulumi-secrets"  # nosec B101
    assert key_calls["resource_name"] == "pulumi-secrets-key-repo"  # nosec B101
    assert (
        key_calls["description"] == "Pulumi secrets KMS key for repo (main)"
    )  # nosec B101
    ...
    assert (
        '"AWS": "arn:aws:iam::123456789012:root"' in key_calls["policy"]
    )  # nosec B101
    ...
    assert (
        alias_calls["target_key_id"] == "pulumi-secrets-key-repo-key-id"
    )  # nosec B101
    assert (
...
def test_mutation_target_github_oidc_role_name_limits_length():
    ...
    assert (
        github_oidc._truncate_role_suffix(repo_suffix) == f"{'a' * 42}-{digest}"
    )  # nosec B101
...
def test_mutation_target_github_oidc_truncation_keeps_digest():
    assert (
        github_oidc._role_name_for_suffix("repo-short") == "PulumiDeploy-repo-short"
    )  # nosec B101
